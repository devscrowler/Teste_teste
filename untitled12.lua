...
local function safeParentScreenGui(sg)
    ...
    if RunService:IsClient() then
        ...
        if not player then
            -- Wait briefly for LocalPlayer to exist (client LocalScript context should have it)
            player = Players:WaitForChild("LocalPlayer", 5) -- may return nil if not present
        end
        if player and typeof(player) == "Instance" then
            ...
            if ok and pg then
                sg.Parent = pg
                LocalPlayer = player
                debugPrint("MirageHubPro: parented to PlayerGui")
                return
            end
        end
